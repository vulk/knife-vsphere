...
class Chef::Knife::BaseVsphereCommand < Knife
  ...
  def get_vim_connection
    ...
    # if it is not in the config file
    conn_opts[:password] = get_password unless conn_opts[:password]
  ...
  def get_vm(vmname)
    ...
    traverse_folders_for_vm(baseFolder, vmname)
  end
  ...
  def get_datacenter
    config[:vim].serviceInstance.find_datacenter(get_config(:vsphere_dc)) or abort "datacenter not found"
  end
  ...
  def find_pool(poolName)
    ...
    entityArray.each do |entityArrItem|
      if entityArrItem != ''
        if RbVmomi::VIM::Folder === baseEntity
        ...
        elsif RbVmomi::VIM::ClusterComputeResource === baseEntity
          baseEntity = baseEntity.resourcePool.resourcePool.find { |f| f.name == entityArrItem } or
              abort "no such pool #{poolName} while looking for #{entityArrItem}"
        elsif RbVmomi::VIM::ResourcePool=== baseEntity
  ...
  def choose_datastore(dstores, size)
    ...
    dstores.each do |store|
      ...
      if store.summary[:freeSpace] - vmdk_size_b > 0
        ...
        candidates << store if cap_remains.to_i > 10
      end
    end
  ...
  def find_datastores_regex(regex)
    ...
    baseEntity.select {|ds| ds.name.match /#{regex}/}
  end
  ...
  def find_device(vm, deviceName)
    vm.config.hardware.device.find { |d| d.deviceInfo.label == deviceName }
  end
